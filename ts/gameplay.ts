...
function handleWinners() {
    ...
    if (numPlayers !== 1) {
        ...
        winners = sortedScores.filter(([player, score]) => score === topScore).map(([player]) => player);

        modal = createModal(modalBg, numPlayers, winners);
        for (let i = 0; i <= numPlayers; i++) {
            createPlayerResult(modal, `Player ${i + 1}`, playerStats[`player_${i + 1}`].score, winners);
        }
    } else {
...
function createModal(parent: HTMLElement, numPlayers: number, winners: string[]) {
    ...
    // let singleWinner = "";
    let winner = winners.length > 1 && numPlayers > 1 ? "It's a tie!" : `${"singleWinner"} Wins!`;
